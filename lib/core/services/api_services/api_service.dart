...
class ApiService {
  ...
  Future<T?> get<T>(
    ...
    try {
      return (await _dio.get<T>(path, queryParameters: query)).data;
  ...
  Future<T?> post<T>({
    ...
    try {
      ...
      if (token != null) {
        ...
        rawToken ? token : "Bearer $token";

        debugPrint("🔐 Custom Authorization Header Added → ${options.headers!["Authorization"]}");
      }
